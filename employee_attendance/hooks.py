...
def _set_cron_nextcall(env):
    def _get_nextcall() -> datetime.datetime:
        ...
        target_nextcall = fields.Datetime.now().astimezone(user_tz).replace(hour=23, minute=59, second=0)
        if fields.Datetime.now().astimezone(user_tz) > target_nextcall: # Check if today nextcall already passed
            target_nextcall = target_nextcall + datetime.timedelta(hours=24)
        return target_nextcall
